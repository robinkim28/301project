...
#include <string>
#include <vector>
/* This class creates any unit that can compute a mathematical function given two inputs, and will output one number. The operation it does is determined by its control. Note that control for this is a string, not 0 or 1
 */
...
class MathUnit
{
 public:
  ...
  //create the math unit with restricted possible math operations. For example, if it's multiplexor, we want the operation to be only f(x1,x2)=x1 or f(x1,x2)=x2, and no other operation, so listOperation = {"0","1"}
  ...
  //with the current 2 inputs and control, do the operation and put result into the output. This does not change input or control the control determines the operation, so the algorithms is a lot of cases (many if-else checking control value)
  void calculate();
