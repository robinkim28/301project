...
// Initializes all the fields for every instruction in Opcode enum
{
  ...
  myArray[SRA].name = "sra"; myArray[SRA].numOps = 3; myArray[SRA].rdPos = 0; myArray[SRA].rsPos = -1; myArray[SRA].rtPos = 1; myArray[SRA].immPos = 2; myArray[SRA].instType = RTYPE; myArray[SRA].op_field = "000000"; myArray[SRA].funct_field = "000011"; myArray[ADD].shamtLabel = true;
...
myArray[BEQ].name = "beq"; myArray[BEQ].numOps = 3; myArray[BEQ].rdPos = -1; myArray[BEQ].rsPos = 0; myArray[BEQ].rtPos = 1; myArray[BEQ].immPos = 2;myArray[BEQ].instType = ITYPE; myArray[BEQ].op_field = "000100"; myArray[BEQ].immLabel = true; 
  myArray[LB].name = "lb"; myArray[LB].numOps = 3; myArray[LB].rdPos = -1; myArray[LB].rsPos = 2; myArray[LB].rtPos = 0; myArray[LB].immPos = 1; myArray[LB].instType = ITYPE; myArray[LB].op_field = "100000"; myArray[LB].offsetLabel = true; 
  ...
  myArray[LW].name = "lw"; myArray[LW].numOps = 3; myArray[LW].rdPos = -1; myArray[LW].rsPos = 2; myArray[LW].rtPos = 0; myArray[LW].immPos = 1; myArray[LW].instType = ITYPE; myArray[LW].op_field = "100011"; myArray[LW].offsetLabel = true; 
  myArray[SW].name = "sw"; myArray[SW].numOps = 3; myArray[SW].rdPos = -1; myArray[SW].rsPos = 2; myArray[SW].rtPos = 0; myArray[SW].immPos = 1; myArray[SW].instType = ITYPE; myArray[SW].op_field = "101011"; myArray[SW].offsetLabel = true; 
}
...
Opcode OpcodeTable::getOpcodeFromBin(string line)
// Given a binary string opcode, check if it exists in the table
{	
	string opField = line.substr(0,6); //first 6 digits
	for(int i = 0; i < (int)UNDEFINED; i++){
		if(opField == "000000")
		{
			string functField = line.substr(26); //last 6 digits for funct field
			if(myArray[i].funct_field == functField)
			{
				return (Opcode)i;
			}
		}
		else if(myArray[i].op_field == opField){
    			return (Opcode)i;
    		}
	}
	  return UNDEFINED;
...
bool OpcodeTable::isShamtLabel(Opcode o)
  // Given an Opcode, returns true if instruction expects a shift in its imm field
{
  if(o < 0 || o > UNDEFINED)
    return false;
  return myArray[o].shamtLabel;
}

bool OpcodeTable::isOffsetLabel(Opcode o)
  // Given an Opcode, returns true if instruction expects prentices for formatting
{
  if(o < 0 || o > UNDEFINED)
    return false;
  return myArray[o].offsetLabel;
}
